...
import com.kkb.b2c.gciantispider.dao.IRealTimeComputDataDao;
...
import com.kkb.b2c.gciantispider.model.NhLinkTrafficInformation;
import com.kkb.b2c.gciantispider.model.RealTimeComputData;
...
public class IRealTimeComputDataServiceImpl implements
	...
	public void saveDataCollectData() {
		...
		for (String serverAddr : keySet) {
			...
			//读取数据库中 server_name与serverAddr相同的数据
			...
			if (datacollect == null) {
				...
				// 历史数据存在，累加
				//近三天的数据=昨天的数据量+前天的数据量+今天的数据量
				datacollect.setLastThreeDaysNum(linkCount.get(serverAddr)
						+ datacollect.getYesterdayNum()
						+ datacollect.getBeforeYesterdayNum());

				//将昨天的数据赋值给前天
				datacollect.setBeforeYesterdayNum(datacollect.getYesterdayNum());
				//将今天的数据赋值给昨天
				datacollect.setYesterdayNum(linkCount.get(serverAddr));
				//更新结果到数据库中
